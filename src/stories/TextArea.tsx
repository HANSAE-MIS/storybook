...
export const TextArea = ({ width=320, field='text field', size='medium', error, errorMessage, onChangeEvent, setDisabled=false}: TextInputProps) => {
    ...
    return (
        <div className='flex flex-col'>
            <div className={`border-[1px] text-[#000000] flex overflow-hidden ${size === 'medium' ? 'h-[34px] text-[14px]' : size === 'small' ? 'h-[140px] text-[13px]' : ''} ${error ? '!border-[#EE3737]' : 'border-[#B3B3B3]'} rounded-[3px]} ${setDisabled && '!border-[#B3B3B3]' }`} style={{width: `${width}px`}}>
            {setDisabled ?
